$(function () {
	...
	function carouselService() {
		$('.carousel-project-item').each(function () {
			...
			if (windowW >= 790 && windowW <= 1024) {
				//ths.css('padding-top', windowH/5);
				console.log('Padding top: ' + windowH/5);
				ths.find('.carousel-project-image').css('min-height', imgH);
				ths.find('.carousel-project-image img').css('max-height', imgH/2.5);
				ths.find('.carousel-project-image').css('max-height', imgH);
			} else {
				ths.find('.carousel-project-image').css('min-height', imgH);
				ths.find('.carousel-project-image img').css('max-height', imgH/1.4);
				ths.find('.carousel-project-image').css('max-height', imgH);
			}
